...
Raw seeds are exploratory. Exercise design cards are the source of truth for exercise batch creation and should use the canonical H4 card contract: stable card ID, status lifecycle (`draft`, `needs-review`, `ready-for-exercise-batch`, `used-in-exercise`, `deferred`, or `rejected`), local scope, target skill, difficulty, exercise role/type, prerequisites, linked-support convention, target ability, decision point, why the exercise deserves to exist, statement shape, expected answer form, intended method, traps, hint ladder, parameter constraints, feasibility sketch, verification strategy, source/provenance, variants, estimated time, potential sets, risks, and readiness note.
...
Ready quiz item design cards should use the canonical H4 item-card contract and the status lifecycle `draft`, `needs-review`, `ready-for-quiz-file`, `used-in-quiz`, `deferred`, or `rejected`. MCQ/MR cards need choices, correct choice(s), distractor rationale, per-choice feedback planning, and misconception mapping before final quiz file creation. Non-choice cards need item-type-specific answer, feedback, verification, remediation, and planning fields without fake per-choice requirements.
...
A candidate golden unit must be initialized and uses the same canonical final-artifact inventory, compact production dashboard, and decision journal as every other initialized or published content unit.
